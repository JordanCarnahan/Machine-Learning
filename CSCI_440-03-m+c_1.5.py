...
#why float64, iso int, iso float32
#numpy.var(a)
...
#recall: cos graph is the same as a sine graph, shifted 90 degrees to the left
...
#it inserts a "singleton" dimension. must specify the dimension or axis where the new dimension should be inserted. helpful for adding a batch #dimension to a single data sample for a neural network.
...
#dim: int, the index at which to insert the singleton vector
#do this example with x=torch.tensor([5,10,15,20])
#y=torch.tensor([16,17,18,19])
